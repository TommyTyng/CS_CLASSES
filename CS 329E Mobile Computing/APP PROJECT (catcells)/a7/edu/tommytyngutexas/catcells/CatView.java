...
public class CatView extends View {
    ...
    private AnimationDrawable mMeow;

    private Mode mMode = Mode.Endless;
    ...
    private void init(Context context) {
        ...
        if (Build.VERSION.SDK_INT>=21) {
            mMeow = (AnimationDrawable) context.getResources().getDrawable(R.drawable.explosion, null);
        } else {
            mMeow = (AnimationDrawable) context.getResources().getDrawable(R.drawable.explosion);
        }
        setAnimationDrawableCallback(mMeow);
        mMeow.setAlpha(180);
    ...
    public void startMeow() {
        ...
        if (mMeasuredSizes.size()>0) {
            ...
            mMeow.setBounds(p.x, p.y, r.right+p.x, r.bottom+p.y);
            setAnimationDrawableCallback(mMeow);
            startAni(mMeow);
    ...
    private void setAnimationDrawableCallback(final AnimationDrawable draw) {
        draw.setCallback(new Drawable.Callback() {
            ...
            public void invalidateDrawable(Drawable drawable) {
                CatView.this.postInvalidate();
            }

            @Override
            public void scheduleDrawable(Drawable drawable, Runnable runnable, long when) {

                CatView.this.postDelayed(runnable, when - SystemClock.uptimeMillis());
            }

            @Override
            public void unscheduleDrawable(Drawable drawable, Runnable runnable) {
                CatView.this.removeCallbacks(runnable);
            }
        });
    }
